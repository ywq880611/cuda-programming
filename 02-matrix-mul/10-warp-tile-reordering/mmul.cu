...
__device__ void loadFromGmem(int iRowA, int iColA, int iRowB, int iColB, int offset, 
int eRow, int eCol, float *a, float *b, float *s_a, float *s_b){
        ...
        for(int j = 0; j < BK; j ++) {
            ...
            for(int wmi = 0; wmi < WMITER; wmi ++){
                for(int ra = 0; ra < TM; ra ++) {
                    regA[wmi * TM + ra] = s_a[(warpRow * WM + wmi * WSUBM + iRowSubC + ra) * BK + j];
                }
            }
            for(int wni = 0; wni < WNITER; wni ++){
                for(int rb = 0; rb < TN; rb ++) {
                    regB[wni * TN + rb] = s_b[j * BN + warpCol * WN + wni * WSUBN + iColSubC + rb];
                }
            }
        ...
        for(int wsubcol = 0; wsubcol < WNITER; wsubcol ++){
            for(int i = 0; i < TM; i ++) {
                for(int j  = 0; j < TN; j += 4){
                    reinterpret_cast<float4 *>(&c[(cRow + warpRow * WM + wsubrow * WSUBM + iRowSubC + i) * N + cCol + warpCol * WN + wsubcol * WSUBN + iColSubC + j])[0] =
                    reinterpret_cast<float4 *>(&tmps[(wsubrow * TM + i) * (TN * WNITER) + wsubcol * TN + j])[0];
                }
            }
        }
    }
