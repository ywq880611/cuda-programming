...
// If we take `blockIdx.y * blockDim.y + threadIdx.y` as row,
// it could make all threads in a wrap to access a same row
// in matrix A, which could improve perf from `26ms vs 350ms`
// on RTX3090
__global__ void matrixMul(int* a, int* b, int* c, int N) {
  ...
  for (; i < N - 3; i += 4) {
    ...
    c[row * N + col] += a[row * N + i + 1] * b[N * (i + 1) + col];
    c[row * N + col] += a[row * N + i + 2] * b[N * (i + 2) + col];
    c[row * N + col] += a[row * N + i + 3] * b[N * (i + 3) + col];
  }
