...
// On RTX 3090, the if we use tmp to avoid alias (or avoid to access cache), it shows `25ms vs 10ms`.
// If we also add `__restrict__` here, we couldn't see more benefits, so I guess `tmp` do same thing
// as `__restrict__`, maybe I could check PTX later?
