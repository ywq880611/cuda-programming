...
// This way is even slower than previous kernel, try to use share
// memory to get a better perf in next kernel.
...
__global__ void sumReuction(float* a, float* r) {
  ...
  for (int stride = 1; stride < blockDim.x; stride *= 2) {
    if (threadId % (2 * stride) == 0) {
      a[offset + threadId] += a[offset + threadId + stride];
    }
    __syncthreads();
  }
  ...
  if (threadId == 0) atomicAdd(r, a[offset]);
}
